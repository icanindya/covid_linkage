...
for eq_class_limit in [1, 3, 5, 10, 15, 20]:
    ...
    with open(RESULT_FILE.format(eq_class_limit), 'w') as wf:
        ...
        for i in range(1, NUM_SIMULATIONS + 1):
            ...
            df_patient['race'] = df_patient['county'].apply(
                lambda x: random.choices([1, 2, 3, 4, 5], weights=racial_weights[x], k=1)[0])
            ...
            df_joined = pd.merge(df_patient_double, df_voter_filtered, on=['county', 'yob', 'gender', 'race'])  # race
            ...
            patient_counts = df_joined.groupby(['county', 'gender', 'age', 'race', 'event_date'])[
                'patient_serial'].nunique()  # race

            voter_counts = df_joined.groupby(['county', 'gender', 'age', 'race', 'event_date'])[
            ...
            matches.columns = ['county', 'gender', 'age', 'race', 'event_date'] + ['patient_counts', 'voter_counts']

            # print(matches)

            matches.to_csv(MATCH_FILE_RACE.format(eq_class_limit, i), index=False)

            print(eq_class_limit, i, num_unique_patients, num_unique_voters, num_links, num_unique_patients_nto1,
